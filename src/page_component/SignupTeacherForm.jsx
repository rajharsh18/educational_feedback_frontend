...
export default function SignupTeacherForm() {
  ...
  const handleSubmit = (event) => {
    ...
    // API Call for Teacher Signup
    fetch('http://127.0.0.1:5000/signup_teacher', {
      method: 'POST',
      headers: {
      ...
      .then(data => {
        if (data.message === 'Teacher registered successfully') {
          ...
          window.location.href = '/signup_teacher';
        } else {
          alert(data.message);
        }
      })
  ...
  return (
    <div className="flex-grow p-8">
      <h2 className="text-2xl mb-4">Sign Up Teacher</h2>
      <form ref={formRef} onSubmit={handleSubmit}>
        ...
        <InputBox type="text" id="subject" name="subject" required={true} label="Subject" />
